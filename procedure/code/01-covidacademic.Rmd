...
rismedextract <- function(records){
  ...
  pubmed_data_full <- data.frame('PMID'=as.character(PMID(records)),
                                 ...
                                 'Journal' = ISOAbbreviation(records), # Changed from "title" to "ISO Abbreviation"
...
Guide for searching: 
- The code searches for covid-related articles based on the search phrase. It is *expected* that the result we get by doing our own search will be very different from the original results given that more covid articles are published in the time between. Hence, please be aware that if you were to replicate the study, you would get different result almost every time you run the code.
...
- All searches are performed twice, once using the `RISmed` pacakge, and the other using the `pubmedR` package. Both fetch data from the same database, but the content is a bit different. We need data from both of the two searches for our analysis. 

- You do not need an API to download article metadata from pubmed, please do not attempt to fill the API parameter.

- If you wish to do your own search, do not forget to change the search parameters, especially the "date" in search 3,4,5. 
...
Alternatively, *run the code below to load the data and see my replication result*. This search was conducted on `Dec-10-2021`.
